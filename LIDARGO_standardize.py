...
warnings.filterwarnings("ignore")  # TODO yikes
...
class LIDARGO:
    ...
    def bin_and_count_angles(self):
        ...
        llimit_azi = (
            floor(self.outputData.azimuth.min(), self.ang_tol) - self.ang_tol / 2
        )
        ...
        llimit_ele = (
            floor(self.outputData.elevation.min(), self.ang_tol) - self.ang_tol / 2
        )
    ...
    def add_attributes(self):
        ...
        self.outputData["range"].attrs[
            "description"
        ] = "Distance from the lidar source."
        ...
        self.outputData["beamID"].attrs[
            "description"
        ] = "Index of the beam within a scan."
        ...
        self.outputData["rws_norm"].attrs[
            "description"
        ] = "Fluctuation of radial wind speed on top of the binned spatio-temporal median. It is used in the dynamic filter."
        self.outputData["rws_norm"].attrs["units"] = "m/s"

        self.outputData["snr_norm"].attrs["long_name"] = "Normalized SNR"
        self.outputData["snr_norm"].attrs[
            "description"
        ] = "Fluctuation of signal-to-noise ratio on top of the binned spatio-temporal median. It is used in the dynamic filter."
        self.outputData["snr_norm"].attrs["units"] = "dB"

        self.outputData["probability"].attrs["long_name"] = "Probability"
        self.outputData["probability"].attrs[
            "description"
        ] = "Value of 2-D p.d.f. in the rws_norm vs snr_norm plane. It is used in the dynamic filter."
        ...
        self.outputData["time"].attrs[
            "description"
        ] = "Timestamp in UTC format for the specific beam."
        ...
        self.outputData["qc_wind_speed"].attrs[
            "description"
        ] = "This variable contains bit-packed integer values, where each bit represents a QC test on the data. Non-zero bits indicate the QC condition given in the description for those bits."
        ...
        for k in self.qc_flag.keys():
            self.outputData["qc_wind_speed"].attrs[
                f"bit_{self.qc_flag[k]}_description"
            ] = ("Value rejected due to " + k + " criterion.")
            self.outputData["qc_wind_speed"].attrs[
                f"bit_{self.qc_flag[k]}_assessment"
            ] = "Bad"
        ...
        self.outputData.attrs["description"] = (
            "AWAKEN Halo XR/XR+ Lidar standardized and quality-controlled data"
        )
        self.outputData.attrs["history"] = (
            "Generated by "
            + getpass.getuser()
            + " on "
            + socket.gethostname()
    ...
    def qc_report(self):
        ...
        plt.title(
            ...
            + " on "
            + datestr(np.nanmean(dt64_to_num(self.inputData["time"])), "%Y-%m-%d")
            + "\n File: "
        ...
        if self.outputData.attrs["scan_mode"] == "Stare":
            ...
            plt.title(
                ...
                + self.inputData.attrs["location_id"]
                + " on "
                + datestr(np.nanmean(time), "%Y-%m-%d")
                + "\n File: "
                + os.path.basename(self.source)
                + "\n"
                + datestr(np.nanmin(time), "%H:%M:%S")
                + " - "
                + datestr(np.nanmax(time), "%H:%M:%S")
            )
