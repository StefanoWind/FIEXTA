...
class LIDARGO:
    ...
    def process_scan(self, make_figures=True, save_file=False, save_path=None, replace=True, process_time=True):
        """
...
def LiSBOA_v7_2(x_exp,mins,maxs,Dn0,sigma,max_iter=None,calculate_stats=False,f=None,order=2,R_max=3,grid_factor=0.25,tol_dist=0.1,max_Dd=1,verbose=True):
    ...
    if calculate_stats:
        ...
        for m in range(max_iter+1):
            ...
            df=f-interpn(tuple(X_vec),avg[m],np.transpose(np.round(x,10)),bounds_error=False,fill_value=np.nan)
        if verbose:
            sys.stdout.flush()
