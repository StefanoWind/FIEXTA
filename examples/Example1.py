...
sys.path.append(pwd)
import gitstuff.LIDARGO.standardize as stand
...
filename2 = os.path.join(
    pwd, "data/Example1/sc1.lidar.z01.b0.20230830.064613.user4.awaken.rhi.nc"
)
...
rws1 = (
    Data["wind_speed"]
    .where(Data["qc_wind_speed"] == 0)
    .where(Data["elevation"] < max_ele)
    .values
)
rws2 = (
    Data["wind_speed"]
    .where(Data["qc_wind_speed"] == 0)
    .where(Data["elevation"] > 180 - max_ele)
    .values
)
...
for rep in range(len(time[0, :])):
    ...
    plt.title(
        ...
        + str(rep)
        + ": "
        + str(time[0, rep])[:-10].replace("T", " ")
        + " - "
        + str(time[-1, rep])[:-10].replace("T", " ")
    )

    fig_name = os.path.join(
        cd,
        "figures/Example1",
        os.path.basename(filename2).replace(".nc", "{i:02d}".format(i=rep) + ".png"),
    )
...
for qc_sel in range(12):
    ...
    if qc_sel == 0:
        ...
        plt.title(
            Data["qc_wind_speed"]
            .attrs["bit_{qc_sel}_description".format(qc_sel=qc_sel)]
            .replace("Value rejected due to ", "")[:-1]
        )
